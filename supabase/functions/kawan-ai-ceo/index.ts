...
function fallbackAnswer(context:Record<string,unknown>,prompt:string,mode:string){
 ...
 const agendaToday=contextList(context,'calendar_today'), agendaTomorrow=contextList(context,'calendar_tomorrow'), approvals=contextList(context,'ceo_approvals'), pipeline=contextList(context,'pipeline_followups'), risks=contextList(context,'project_risks');
...
Deno.serve(async req=>{
 ...
 const context={...rawContext,calendar_today:agenda.today,calendar_tomorrow:agenda.tomorrow};
 ...
 const cited=[...sources({today_tasks:context.today_tasks,completed_today_tasks:context.completed_today_tasks,overdue_tasks:context.overdue_tasks,calendar_today:context.calendar_today,calendar_tomorrow:context.calendar_tomorrow,ceo_approvals:context.ceo_approvals,pipeline_followups:context.pipeline_followups,project_risks:context.project_risks,unread_notifications:context.unread_notifications}),...executed.filter(item=>item.url).map(item=>({label:item.label,url:item.url}))].filter((item,index,all)=>all.findIndex(value=>value.url===item.url)===index).slice(0,8);
