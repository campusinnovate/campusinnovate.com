...
async function executeAction(client: ReturnType<typeof createClient>, authorization:string, action:Record<string,unknown>, command:string) {
  ...
  if (!['create_task','update_pipeline','save_project_task','save_project_record','complete_activity','create_calendar_meeting','update_activity_due'].includes(type)) return null;
  if(type==='update_activity_due'){
    const activityId=clean(payload.activity_id,64);
    const dueDate=clean(payload.due_date,10);
    if(!activityId||!/^\d{4}-\d{2}-\d{2}$/.test(dueDate))throw new Error('Task dan tanggal deadline wajib jelas.');
    const result=await client.rpc('kawan_ai_ceo_update_activity_due',{target_activity_id:activityId,target_due_date:dueDate,command_text:command});
    if(result.error)throw new Error(result.error.message);
    const output=result.data&&typeof result.data==='object'?result.data as Record<string,unknown>:{};
    return {type,label:clean(output.label,140)||'Deadline task diperbarui',url:clean(output.url,500)};
  }
...
Deno.serve(async req=>{
 ...
 const replyRequested=clean(body.reply_to_message_id,64);
 const replyContext=clean(body.reply_context,3000);
 const replyMessageId=replyRequested&&Array.isArray(history)&&history.some((item:Record<string,unknown>)=>String(item.id)===replyRequested)?replyRequested:null;
 const recentHistory=Array.isArray(history)?history.slice(-8).map((item:Record<string,unknown>)=>`${clean(item.role,16)}: ${clean(item.content,360)}`).join('\n').slice(-2600):'';
 ...
 const input=mode==='morning_briefing'
   ...
   : prompt;
 const response=GROQ_KEY?await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${GROQ_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:MODEL,max_completion_tokens:1800,messages:[{role:'system',content:instruction},{role:'user',content:`Pertanyaan: ${input}${replyContext?`\n\nPesan yang dibalas (jadikan konteks utama): ${replyContext}`:''}${recentHistory?`\n\nRiwayat terbaru: ${recentHistory}`:''}\n\nKonteks terotorisasi ringkas: ${JSON.stringify(compactContext(context)).slice(0,12000)}`}],response_format:{type:'json_object'}})}):null;
 ...
 if(response?.ok){
   ...
   try{const parsed=JSON.parse(String(output??''));answer=clean(parsed.answer,6000);actions=Array.isArray(parsed.actions)?parsed.actions.filter((item:unknown)=>item&&typeof item==='object').slice(0,12) as Array<Record<string,unknown>>:[];}catch{console.error('Groq returned unreadable output',raw);}
 ...
 if(mode==='chat')await client.from('kawan_ai_ceo_messages').insert({membership_id:membership,role:'user',message_kind:'chat',content:prompt,sources:[],parent_message_id:replyMessageId});
 const saved=await client.from('kawan_ai_ceo_messages').insert({membership_id:membership,role:'assistant',message_kind:mode==='morning_briefing'?'morning_briefing':'chat',content:answer,sources:cited,briefing_date:mode==='morning_briefing'?today:null,parent_message_id:replyMessageId}).select().single();
