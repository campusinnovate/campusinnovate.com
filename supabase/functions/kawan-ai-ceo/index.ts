...
function compactList(items:Array<Record<string,unknown>>,limit=50){return items.slice(0,limit).map((item,index)=>`${index+1}. ${itemTitle(item)}${itemDate(item)?` — ${itemDate(item)}`:''}`).join('\n');}
