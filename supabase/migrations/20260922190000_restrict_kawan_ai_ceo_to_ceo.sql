-- Restrict Kawan AI — Asisten CEO to explicitly enabled CEO memberships.
create table if not exists public.kawan_ai_ceo_members (
  membership_id uuid primary key references public.memberships(id) on delete cascade,
  enabled boolean not null default true,
  created_at timestamptz not null default now()
);

alter table public.kawan_ai_ceo_members enable row level security;
revoke all on table public.kawan_ai_ceo_members from anon, authenticated, public;

insert into public.kawan_ai_ceo_members(membership_id,enabled)
select id,true from public.memberships
where email::text='mfauzanramdani4@gmail.com' and status='active'
on conflict(membership_id) do update set enabled=true;

create or replace function public.kawan_ai_ceo_access()
returns boolean language sql stable security definer set search_path=public as $$
  select public.current_membership_id() is not null
    and public.current_user_has_permission('ai.use')
    and exists (
      select 1 from public.kawan_ai_ceo_members cm
      where cm.membership_id=public.current_membership_id() and cm.enabled
    );
$$;

revoke all on function public.kawan_ai_ceo_access() from anon, public;
grant execute on function public.kawan_ai_ceo_access() to authenticated;
...
begin
  ...
  return jsonb_build_object(
    ...
    'pipeline_followups',coalesce((select jsonb_agg(x order by x->>'activity_date') from jsonb_array_elements(feed) x where x->>'feed_kind'='pipeline' and coalesce(x->>'status','not_started')<>'done'),'[]'::jsonb),
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object('id',id,'role',role,'message_kind',message_kind,'content',content,'sources',sources,'briefing_date',briefing_date,'created_at',created_at) order by created_at)
    from (select * from public.kawan_ai_ceo_messages where membership_id=me order by created_at desc limit 60) recent
  ),'[]'::jsonb);
end;
$$;

drop policy if exists "Members can read their CEO AI conversation" on public.kawan_ai_ceo_messages;
create policy "Only enabled CEO can read their AI conversation"
on public.kawan_ai_ceo_messages for select to authenticated
using (membership_id=public.current_membership_id() and public.kawan_ai_ceo_access());

drop policy if exists "Members can write their CEO AI conversation" on public.kawan_ai_ceo_messages;
create policy "Only enabled CEO can write their AI conversation"
on public.kawan_ai_ceo_messages for insert to authenticated
with check (membership_id=public.current_membership_id() and public.kawan_ai_ceo_access());
