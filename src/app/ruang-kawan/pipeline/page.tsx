...
export default function PipelinePage(){
  ...
  const visible=useMemo(()=>leads.filter(lead=>{
    ...
    return (!sourceFilter||lead.source_id===sourceFilter)&&(stageFilter==='all'||lead.stage===stageFilter)&&(!term||[lead.account_name,lead.contact_name,lead.lead_code,lead.business_unit,lead.next_action].some(value=>value?.toLowerCase().includes(term)));
  }),[leads,sourceFilter,stageFilter,query]);
  ...
  if(state==='denied')return <main className="rk-dashboard-foundation"><section className="rk-access-denied"><h1>Pipeline BD belum tersedia</h1><p>Hubungi administrator untuk mengaktifkan akses.</p><Link href="/ruang-kawan/dashboard/">Kembali</Link></section></main>;
  return <main className="rk-pipeline-foundation"><section className="rk-pipeline-shell">
    <nav className="rk-pipeline-nav"><Link href="/ruang-kawan/dashboard/"><FiArrowLeft/> Dashboard</Link><button onClick={()=>void load()}><FiRefreshCw/> Muat ulang</button></nav>
    <header className="rk-pipeline-heading"><div><small>Revenue workspace</small><h1>Pipeline Business Development</h1><p>Kelola lead, next action, follow-up, meeting, proposal, dan closing dalam satu funnel.</p></div>{canManage?<button onClick={startCreate}><FiPlus/> Tambah lead</button>:null}</header>
...
function LeadCard({lead,stages,canEdit,onEdit,onUpdate}:{lead:Lead;stages:string[];canEdit:boolean;onEdit:(lead:Lead)=>void;onUpdate:(lead:Lead,patch:Partial<Pick<Lead,'stage'|'next_action'|'due_date'>>)=>Promise<void>}){
  ...
  return <article className="rk-pipeline-card" style={{'--pipeline-color':lead.source_color} as CSSProperties}>
