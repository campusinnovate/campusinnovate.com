...
export default function LoginPanel() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      if (data.session) window.location.replace('/ruang-kawan/dashboard/');
    });
  }, []);
  ...
  async function loginWithGoogle() {
    ...
    const { error: loginError } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/ruang-kawan/callback/`,
      },
    });
  ...
  return (
    <div className="rk-login-panel">
      ...
      <form onSubmit={loginWithPassword}>
        ...
        {message && !error ? <p className="rk-login-message">{message}</p> : null}
