...
type ModuleConfig = {
  ...
  project_types?: string[]; phases?: string[]; statuses?: string[];
};
type Source = {
  ...
  is_active: boolean; sort_order: number; module_type: 'activity' | 'content_plan' | 'pipeline' | 'project'; module_config: ModuleConfig;
};
    ...
    <section className="rk-admin-layout">
      <form className="rk-member-form" onSubmit={saveSource}>
        ...
        <div className="rk-form-grid">
          ...
          <label>Tipe modul<select value={form.module_type} onChange={(event) => setForm({ ...form, module_type: event.target.value as FormState['module_type'] })}><option value="activity">Aktivitas umum</option><option value="content_plan">Content Plan</option><option value="pipeline">Pipeline BD</option><option value="project">Project Management</option></select></label>
        </div>
        ...
        {form.module_type === 'project' ? <fieldset><legend>Konfigurasi Project Management</legend><p className="rk-field-hint">Admin dapat menambah sumber, tipe, phase, dan status project tanpa perubahan backend.</p><div className="rk-form-grid"><label>Tipe project<textarea value={(form.module_config.project_types ?? []).join('\n')} onChange={(event) => updateConfigList('project_types', event.target.value)} required /></label><label>Phase workflow<textarea value={(form.module_config.phases ?? []).join('\n')} onChange={(event) => updateConfigList('phases', event.target.value)} required /></label><label>Status project<textarea value={(form.module_config.statuses ?? []).join('\n')} onChange={(event) => updateConfigList('statuses', event.target.value)} /></label></div></fieldset> : null}
