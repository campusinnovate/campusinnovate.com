...
import { FiArrowLeft, FiBriefcase, FiCreditCard, FiEdit3, FiPhone, FiRefreshCw, FiSearch, FiShield, FiUser, FiUsers, FiX } from 'react-icons/fi';
...
export default function EmployeeProfilePage() {
  ...
  async function load(preferredId?: string) {
    ...
    const {data,error:loadError}=await supabase.rpc('employee_directory');
    if(loadError){setState('denied');return;}
  ...
  return <main className="rk-profile-foundation"><section className="rk-profile-shell">
    ...
    <header className="rk-profile-heading"><div><small>People directory</small><h1>Profil Pegawai</h1><p>Profil kerja, kontak internal, informasi administratif, dan rekening anggota Campus Innovate.</p></div><span><FiUsers/><b>{members.length}</b><small>anggota aktif</small></span></header>
    ...
    <section className="rk-profile-layout">
      ...
      <section className="rk-profile-detail">{selected&&form?<>
        ...
        {!editing?<div className="rk-profile-summary">
          ...
          <article className="rk-profile-bank"><header><FiCreditCard/><h3>Rekening pembayaran</h3></header><strong>{selected.bank_name||'Bank belum diisi'}</strong><code>{selected.bank_account_number||'—'}</code><p>{selected.bank_account_holder||'Nama pemilik belum diisi'}{selected.bank_branch?` · ${selected.bank_branch}`:''}</p></article>
