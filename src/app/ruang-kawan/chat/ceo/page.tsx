...
import { FiAlertCircle, FiArrowLeft, FiCalendar, FiCheckCircle, FiChevronRight, FiClock, FiExternalLink, FiFileText, FiLoader, FiPaperclip, FiSend, FiShield, FiSparkles } from 'react-icons/fi';
...
export default function KawanAiCeoPage(){
 ...
 return <main className="rk-ceo-foundation"><section className="rk-ceo-shell">
  <aside className="rk-ceo-sidebar">
   <div className="rk-ceo-brand"><span><FiSparkles/></span><div><small>RUANG KAWAN</small><strong>Kawan Chat</strong></div></div>
   ...
   <Link className="rk-ceo-assistant" href="/ruang-kawan/chat/ceo/"><FiSparkles/><span><strong>Kawan AI — Asisten CEO</strong><small>Prioritas, deadline &amp; approval</small></span></Link>
  ...
  <section className="rk-ceo-main">
   ...
   <section className="rk-ceo-content">
    <article className="rk-ceo-briefing"><div><span className="rk-ceo-kicker"><FiSparkles/> BRIEFING CEO</span><h2>Selamat datang, Fauzan 👋</h2><p>Berikut ringkasan yang perlu Anda perhatikan hari ini.</p></div><div className="rk-ceo-metrics">
    ...
    <section className="rk-ceo-suggestions"><strong><FiSparkles/> Coba tanyakan ini:</strong><div>{prompts.map(value=><button type="button" key={value} onClick={()=>void ask(value)} disabled={sending}>{value}</button>)}</div></section>
