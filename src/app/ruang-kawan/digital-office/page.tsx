...
export default function DigitalOffice() {
  ...
  async function send() {
    ...
    try {
      ...
      const result = await client.rpc('create_office_document', { payload: { id, title: title.trim(), source_path: path, source_hash: await sha256(bytes), page_count: pages, signers: placements } }); if (result.error) throw result.error;
      ...
      notify({ title: 'Permintaan tanda tangan terkirim', message: 'Setiap pemilik harus menyetujui lokasi dan dokumen, termasuk tanda tangan milik sendiri.', kind: 'success' });
  ...
  async function finalize() {
    ...
    try {
      ...
      const response=await fetch(`${supabaseUrl}/functions/v1/digital-office/finalize`,{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({documentId:selected.id})});
      const result=await response.json();if(!response.ok)throw new Error(result.error||'Finalisasi PDF belum tersedia.');
      const docs = await load(); setSelected(docs?.find(d => d.id === selected.id) ?? selected);
      notify({ title: 'PDF bertanda tangan siap diunduh', message: 'QR kredensial dan catatan persetujuan tersedia.', kind: 'success' });
