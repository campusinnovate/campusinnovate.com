...
import { FiActivity,FiBarChart2,FiBell,FiBookOpen,FiBriefcase,FiChevronDown,FiDollarSign,FiFileText,FiGrid,FiLogOut,FiMoreHorizontal,FiSettings,FiTrendingUp,FiUser } from 'react-icons/fi';
...
export default function WorkspaceMiniNav(){
 const pathname=usePathname();const[access,setAccess]=useState<Access|null>(null);const[email,setEmail]=useState('');const[hidden,setHidden]=useState(false);const[profileOpen,setProfileOpen]=useState(false);const[moreOpen,setMoreOpen]=useState(false);const lastScroll=useRef(0);const navRef=useRef<HTMLElement>(null);
 ...
 </div></nav><nav className="rk-mobile-nav" aria-label="Navigasi mobile Ruang Kawan">{mobile.map(item=>{const Icon=item.icon;return <Link key={item.href} href={item.href} data-active={activeFor(item.href)}><Icon/><span>{item.label==='My Activity'?'Activity':item.label}</span></Link>})}{access.permissions.includes('notifications.view_self')?<Link href="/ruang-kawan/notifications/" data-active={pathname.startsWith('/ruang-kawan/notifications')}><FiBell/><span>Notifikasi</span></Link>:null}<Link href="/ruang-kawan/profile/" data-active={pathname.startsWith('/ruang-kawan/profile')}><FiUser/><span>Profil</span></Link></nav></>
}
